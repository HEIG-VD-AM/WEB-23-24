...
// Configuration de la session et de Passport (ne pas prendre en compte la sécurité du secret)
app.use(session({ secret: 'A_VERY_VERY_SECURE_SECRET', resave: false, saveUninitialized: false }));
...
// Mise à jour de la route de connexion
...
function isLoggedIn(req, res, next) {
    ...
    res.redirect('/login?error=You must first log in');
}
...
app.use('/conversation/:conversationId', isLoggedIn, async (req, res, next) => {
    ...
    getConversationById(req.params.conversationId).then(
        (conversation) => {
            console.log(`Conversation ${conversation} found`)
