...
export class Game extends Map {
    ...
    moveShape(id, col) {
        ...
        if (!shape) {
            console.log("Cannot find shape " + id + "; ignoring");
            return;
        }
    ...
    rotateShape(id, rotation) {
        ...
        const len = shapeTypes[shape.shapeType].length;
        let newRotation = (rotation == "left" ? shape.rotation - 1 : shape.rotation + 1);
        newRotation = ((newRotation % len) + len) % len;

        console.log("Actual rotation : ", newRotation);

        if (this.map.testShape(shape, shape.row, shape.col, newRotation)) {
    ...
    onMessage(playerId, message) {
        // handle depending on the class of the message
